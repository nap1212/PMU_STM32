...
|:---:|:---:|:---:|:---|
| Pin 1 | 3.3V | 3.3V | 電源（**5V 不可・必ず 3.3V へ**） |
| Pin 6 | GND | GND | グランド |
| Pin 8 | TXD | **Pin19 (RX1)** | GPS データ出力 → Mega 受信 |
| Pin 10 | RXD | **Pin18 (TX1)** | Mega 送信 → GPS（レベル変換推奨） |
| Pin 7 | PPS | Pin2 | 1PPS パルス（任意・割り込み対応ピン） |

> **注意**: TXD/RXD/PPS は 3.3V レベルです。  
> Arduino Mega は 5V 動作なので Pin18(TX1)→GPS RXD の経路にレベル変換が必要です。
